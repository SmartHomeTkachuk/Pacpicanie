...
Версия 5.12 – абсолютно финальная, все известные проблемы исправлены.
"""
...
async def get_group_list(session: aiohttp.ClientSession, force_refresh: bool = False) -> List[str]:
    ...
    async with _groups_cache_lock:
        ...
        # Если не принудительно, пробуем кеш
...
async def select_group(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Если список групп пуст, пробуем обновить его принудительно
    if not all_groups:
...
def main():
    ...
    try:
        _global_loop.run_until_complete(app.run_polling(allowed_updates=["message", "callback_query"]))
    except (KeyboardInterrupt, SystemExit):
        pass
    finally:
        if not _global_loop.is_closed():
            _global_loop.run_until_complete(shutdown_app(app))
            _global_loop.close()
